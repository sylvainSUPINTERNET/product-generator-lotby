...
@click.option("--image_url", prompt="Url image (not uploaded to stripe !)", required=True)
...
def create_product_stripe(name, description, image_url, shippable, statement_descriptor, remove_image_background, unit_amount_decimal, currency, tax_code, total_product_price, days_before_game_end_at, max_slot):
    ...
    try:
        ...
        price = stripe.Price.create(
            product=product.id,
            unit_amount_decimal=str(int( float(unit_amount_decimal) * 100 )), # 2000 means 20.00 for stripe, so we need to convert str to float then multiply and convert back to int
